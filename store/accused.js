...
export async function fetchAccusedList($axios) {
  try {
    const { data } = await $axios.get(
      `${process.env.rest_api_path}/user/get_all_accuseds`
    )
...
export async function createAccusedItem({ $axios, data }) {
  try {
    await $axios.put(`${process.env.rest_api_path}/admin/put_accused`, data)
...
export async function updateAccusedItem({ $axios, id, data }) {
  try {
    await $axios.patch(
      `${process.env.rest_api_path}/admin/update_accused`,
      data
    )
...
export async function deleteAccusedItem({ $axios, id }) {
  try {
    await $axios.delete(
      `${process.env.rest_api_path}/admin/delete_accused?id=${id}`
    )
